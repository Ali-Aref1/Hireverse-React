...
export const UserMenu = ({showMenu, setShowMenu, user, setUser}:UserMenuProps) => {
  ...
  return (
    ...
    <Link to={`/profile/${user.id}`}><button className="w-full text-white hover:bg-gray-700 px-3 py-2 text-sm transition-colors font-bold" onClick={()=>{setShowMenu(false)}}>Profile</button></Link>
    <div className="w-full h-0 border-2 border-slate-900"></div>
